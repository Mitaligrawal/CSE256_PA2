...
def main():
    ...
    # Initialize models
    encoder = TransformerEncoder(tokenizer.vocab_size, n_embd=n_embd, n_head=n_head, n_layer=n_layer, max_seq_len=block_size).to(device)
    classifier = FeedforwardClassifier(n_input=n_embd, n_hidden=n_hidden, n_output=n_output).to(device)
    ...
    for epoch in range(epochs_CLS):
        ...
        for xb, yb in train_CLS_loader:
            ...
            logits = classifier(pooled)              # (batch_size, n_output)
        ...
        acc = compute_classifier_accuracy(encoder, classifier, train_CLS_loader)
        test_acc = compute_classifier_accuracy(encoder, classifier, test_CLS_loader)
        test_accuracies.append(test_acc)
        print(f"Epoch {epoch+1}/{epochs_CLS} | Loss: {avg_loss:.4f} | Train Accuracy: {acc:.2f}% | Test Accuracy: {test_acc:.2f}%")

    print("\nFinal Test Accuracy: {:.2f}%".format(test_accuracies[-1]))

    # Report number of parameters in encoder
    n_params = sum(p.numel() for p in encoder.parameters() if p.requires_grad)
    print(f"Number of parameters in encoder: {n_params}")
